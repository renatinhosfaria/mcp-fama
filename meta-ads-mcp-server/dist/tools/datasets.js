...
export function registerDatasetTools(server) {
    server.registerTool('meta_get_dataset_details', {
        title: 'Detalhes do Dataset / Pixel',
        description: 'Retorna detalhes completos de um Pixel do Meta (dataset CAPI): ' +
        ...
        try {
            if (pixel_id) {
                const pixel = await makeRequest(pixel_id, 'GET', { fields: PIXEL_FIELDS });
                const result = {
                    id: pixel.id,
                    name: pixel.name,
            ...
            const pixels = await paginate(`${accountId}/adspixels`, { fields: PIXEL_FIELDS }, 10);
    ...
    server.registerTool('meta_get_dataset_quality', {
        title: 'Qualidade do Dataset / Pixel',
        description: 'Retorna métricas de qualidade dos eventos recebidos por um Pixel do Meta. ' +
        ...
        try {
            const response = await makeRequest(`${pixel_id}/data_quality`, 'GET', { fields: 'event_name,custom_event_type,connected_assets_quality,event_match_quality' });
            const result = {
                pixel_id,
    ...
    server.registerTool('meta_get_dataset_stats', {
        title: 'Estatísticas do Dataset / Pixel',
        description: 'Retorna estatísticas de volume de eventos de um Pixel do Meta em um período. ' +
        ...
        try {
            ...
            const response = await makeRequest(`${pixel_id}/stats`, 'GET', params);
            const result = {
                pixel_id,
                aggregation,
                data: response.data || [],
