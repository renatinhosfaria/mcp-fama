...
import { z } from 'zod';
import { handleApiError, makeMultipartRequest, makeRequest, normalizeAdAccountId, paginate, toGraphArrayParam, toJson, } from '../client.js';
...
export async function normalizeUploadSource(input) {
...
export function registerAssetTools(server) {
    server.registerTool('meta_list_ad_images', {
        ...
        try {
            const images = await paginate(`${resolveAccountId(ad_account_id)}/adimages`, { fields: AD_IMAGE_FIELDS }, limit);
            const result = {
                total: images.length,
    ...
    server.registerTool('meta_get_ad_image', {
        ...
        try {
            let image;
            if (image_id) {
                image = await makeRequest(image_id, 'GET', { fields: AD_IMAGE_FIELDS });
            }
            else {
                const response = await makeRequest(`${resolveAccountId(ad_account_id)}/adimages`, 'GET', {
    ...
    server.registerTool('meta_upload_ad_image', {
        ...
        try {
            ...
            const response = await makeRequest(`${resolveAccountId(ad_account_id)}/adimages`, 'POST', {}, {
    ...
    server.registerTool('meta_list_ad_videos', {
        title: 'Listar Vídeos',
        ...
        try {
            const videos = await paginate(`${resolveAccountId(ad_account_id)}/advideos`, { fields: AD_VIDEO_FIELDS }, limit);
            const result = {
                total: videos.length,
    ...
    server.registerTool('meta_get_ad_video', {
        ...
        try {
            const video = await makeRequest(video_id, 'GET', { fields: AD_VIDEO_FIELDS });
    ...
    server.registerTool('meta_get_ad_video_status', {
        ...
        try {
            const video = await makeRequest(video_id, 'GET', { fields: 'id,status,created_time,updated_time' });
            const result = {
                id: video.id,
    ...
    server.registerTool('meta_upload_ad_video', {
        title: 'Upload de Vídeo',
        description: 'Faz upload de vídeo para a conta de anúncios. ' +
        ...
        try {
            ...
            if (file_url && !file_path && !base64_data) {
                response = await makeRequest(`${accountId}/advideos`, 'POST', {}, {
                    file_url,
                    ...(title ? { title } : {}),
                ...
                response = await makeMultipartRequest(`${accountId}/advideos`, formData, {}, { timeout: 120000 });
            }
            const result = {
                success: true,
                response,
