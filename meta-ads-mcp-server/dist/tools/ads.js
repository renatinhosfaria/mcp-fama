...
export function registerAdTools(server) {
    server.registerTool('meta_list_ads', {
        title: 'Listar Anúncios',
        description: 'Lista anúncios de uma conta, campanha ou ad set específico. ' +
        ...
        try {
            ...
            const ads = await paginate(endpoint, params, limit);
            const result = {
                total: ads.length,
    ...
    server.registerTool('meta_get_ad', {
        title: 'Detalhes do Anúncio',
        description: 'Retorna detalhes completos de um anúncio: status, criativo, ad set pai, ' +
        ...
        try {
            const ad = await makeRequest(ad_id, 'GET', { fields: AD_FIELDS });
            return {
                content: [{ type: 'text', text: toJson(formatAd(ad)) }],
    ...
    server.registerTool('meta_create_ad', {
        title: 'Criar Anúncio',
        description: 'Cria um novo anúncio dentro de um ad set. ' +
        ...
        inputSchema: z.object({
            ...
            creative: z
                .string()
                .describe('JSON do criativo. Use {"creative_id":"EXISTING_ID"} para reutilizar um criativo, ' +
        ...
        try {
            ...
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: 'text', text: 'Erro: O campo creative deve ser um JSON válido.' }],
                };
            }
            const data = {
                name,
            ...
            if (tracking_specs) {
                try {
                    ...
                    return {
                        isError: true,
                        content: [{ type: 'text', text: 'Erro: tracking_specs deve ser um JSON válido.' }],
                    };
                }
            }
            const response = await makeRequest(`${accountId}/ads`, 'POST', {}, data);
            const result = {
                success: true,
                ad_id: response.id,
    ...
    server.registerTool('meta_update_ad', {
        title: 'Atualizar Anúncio',
        description: 'Atualiza um anúncio existente: nome, status, criativo ou bid. ' +
        ...
        try {
            ...
            if (bid_amount !== undefined)
                data.bid_amount = bid_amount;
            if (conversion_domain !== undefined)
                data.conversion_domain = conversion_domain;
            if (creative !== undefined) {
                try {
                    ...
                    return {
                        isError: true,
                        content: [{ type: 'text', text: 'Erro: creative deve ser um JSON válido.' }],
                    };
                }
            }
            ...
            await makeRequest(ad_id, 'POST', {}, data);
            const result = {
                success: true,
                ad_id,
    ...
    server.registerTool('meta_delete_ad', {
        title: 'Deletar Anúncio',
        description: 'Deleta um anúncio permanentemente. ' +
        ...
        try {
            await makeRequest(ad_id, 'DELETE');
            const result = {
                success: true,
                ad_id,
