...
export function registerAdLibraryTools(server) {
    ...
    }).strict().refine((input) => input.search_terms.trim().length > 0 || Boolean(input.search_page_ids?.length), {
        message: 'Informe search_terms ou search_page_ids.',
        path: ['search_terms'],
    });
    server.registerTool('meta_search_ad_library', {
        title: 'Pesquisar Ad Library',
        description: 'Pesquisa anúncios na biblioteca pública da Meta via Ads Archive. ' +
        ...
        try {
            ...
            const response = await makeRequest('ads_archive', 'GET', params);
...
function formatArchiveResult(item) {
    return {
        id: item.id || null,
