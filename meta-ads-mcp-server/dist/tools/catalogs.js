import { z } from 'zod';
import { makeRequest, paginate, handleApiError, toJson, } from '../client.js';
...
export function registerCatalogTools(server) {
    server.registerTool('meta_catalog_create', {
        title: 'Criar Catálogo de Produtos',
        description: 'Cria um novo Catálogo de Produtos no Business Manager. ' +
        ...
        try {
            ...
            const response = await makeRequest(`${bid}/owned_product_catalogs`, 'POST', {}, { name, vertical });
            const result = {
                success: true,
    ...
    server.registerTool('meta_catalog_get_catalogs', {
        title: 'Listar Catálogos de Produtos',
        description: 'Lista todos os Catálogos de Produtos de um Business Manager. ' +
        ...
        try {
            const bid = resolveBusinessId(business_id);
            const catalogs = await paginate(`${bid}/owned_product_catalogs`, { fields: CATALOG_FIELDS }, limit);
    ...
    server.registerTool('meta_catalog_get_details', {
        title: 'Detalhes do Catálogo de Produtos',
        description: 'Retorna detalhes completos de um Catálogo de Produtos: ' +
        ...
        try {
            const catalog = await makeRequest(catalog_id, 'GET', { fields: CATALOG_FIELDS });
            return {
                content: [{ type: 'text', text: toJson(catalog) }],
    ...
    server.registerTool('meta_catalog_get_diagnostics', {
        title: 'Diagnóstico do Catálogo de Produtos',
        description: 'Retorna erros e avisos de um Catálogo de Produtos: ' +
        ...
        try {
            const response = await makeRequest(`${catalog_id}/diagnostics`, 'GET', { fields: 'affected_channels,affected_features,affected_entities,diagnostics' });
    ...
    server.registerTool('meta_catalog_get_feed_rules', {
        title: 'Regras do Feed de Produtos',
        description: 'Retorna as regras de transformação de um feed de produtos de catálogo. ' +
        ...
        try {
            const response = await makeRequest(`${feed_id}/rules`, 'GET', { fields: 'attribute,params,rule_type' });
    ...
    server.registerTool('meta_catalog_get_products', {
        title: 'Listar Produtos do Catálogo',
        description: 'Lista produtos de um Catálogo de Produtos. ' +
        ...
        try {
            const params = { fields: PRODUCT_FIELDS };
            if (filter_by)
                params.filter_by = filter_by;
            const products = await paginate(`${catalog_id}/products`, params, limit);
    ...
    server.registerTool('meta_catalog_get_product_details', {
        title: 'Detalhes de Produto do Catálogo',
        description: 'Retorna todos os campos de um produto específico do catálogo: ' +
        ...
        try {
            const product = await makeRequest(product_id, 'GET', { fields: PRODUCT_FIELDS });
            return {
                content: [{ type: 'text', text: toJson(product) }],
    ...
    server.registerTool('meta_catalog_get_product_feed_details', {
        title: 'Feeds de Produtos do Catálogo',
        description: 'Lista os feeds de produtos de um catálogo: nome, agendamento de importação, ' +
        ...
        try {
            const feeds = await paginate(`${catalog_id}/product_feeds`, { fields: FEED_FIELDS }, limit);
    ...
    server.registerTool('meta_catalog_get_product_sets', {
        title: 'Conjuntos de Produtos do Catálogo',
        description: 'Lista os conjuntos de produtos (Product Sets) de um catálogo. ' +
        ...
        try {
            const sets = await paginate(`${catalog_id}/product_sets`, { fields: PRODUCT_SET_FIELDS }, limit);
    ...
    server.registerTool('meta_catalog_get_product_set_products', {
        title: 'Produtos de um Conjunto de Produtos',
        description: 'Lista os produtos de um Product Set específico de catálogo. ' +
        ...
        try {
            const products = await paginate(`${product_set_id}/products`, { fields: PRODUCT_FIELDS }, limit);
