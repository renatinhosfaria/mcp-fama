...
export function registerRuleTools(server) {
    server.registerTool('meta_list_ad_rules', {
        title: 'Listar Regras Automáticas',
        description: 'Lista todas as regras automáticas (Automated Rules) de uma conta de anúncio. ' +
        ...
        try {
            ...
            const rules = await paginate(`${accountId}/adrules_library`, { fields: RULE_FIELDS }, limit);
    ...
    server.registerTool('meta_create_ad_rule', {
        title: 'Criar Regra Automática',
        description: 'Cria uma regra automática que monitora métricas e executa ações automaticamente. ' +
        ...
        inputSchema: z.object({
            ...
            evaluation_spec: z
                .string()
                .describe('JSON com a condição de avaliação. Exemplo: ' +
            ...
            execution_spec: z
                .string()
                .describe('JSON com a ação a executar. Exemplo para pausar: \'{"execution_type":"PAUSE"}\'. ' +
        ...
        try {
            ...
            const response = await makeRequest(`${accountId}/adrules_library`, 'POST', {}, data);
            const result = {
                success: true,
                rule_id: response.id,
    ...
    server.registerTool('meta_list_ad_rules_history', {
        title: 'Histórico de Execuções de Regras',
        description: 'Lista o histórico de execuções das regras automáticas: quando cada regra foi avaliada, ' +
        ...
        try {
            ...
            const history = await paginate(`${accountId}/adrules_history`, params, limit);
