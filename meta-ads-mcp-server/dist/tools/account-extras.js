...
export function registerAccountExtrasTools(server) {
    server.registerTool('meta_list_instagram_accounts', {
        title: 'Listar Contas do Instagram',
        description: 'Lista as contas do Instagram conectadas a uma conta de anúncio. ' +
        ...
        try {
            ...
            const accounts = await paginate(`${accountId}/instagram_accounts`, { fields: 'id,username,name,profile_pic,followers_count,media_count,biography,website' }, limit);
    ...
    server.registerTool('meta_list_saved_audiences', {
        title: 'Listar Públicos Salvos',
        description: 'Lista os públicos salvos (Saved Audiences) de uma conta de anúncio. ' +
        ...
        try {
            const accountId = resolveAccountId(ad_account_id);
            const audiences = await paginate(`${accountId}/saved_audiences`, { fields: 'id,name,description,targeting,run_status,time_created,time_updated,sentence_lines' }, limit);
    ...
    server.registerTool('meta_list_ad_labels', {
        title: 'Listar Labels de Anúncios',
        description: 'Lista os labels (etiquetas) criados na conta de anúncio. ' +
        ...
        try {
            const accountId = resolveAccountId(ad_account_id);
            const labels = await paginate(`${accountId}/adlabels`, { fields: 'id,name,created_time,updated_time' }, limit);
    ...
    server.registerTool('meta_create_ad_label', {
        title: 'Criar Label de Anúncio',
        description: 'Cria um novo label (etiqueta) na conta de anúncio para organizar campanhas e anúncios. ' +
        ...
        try {
            const accountId = resolveAccountId(ad_account_id);
            const response = await makeRequest(`${accountId}/adlabels`, 'POST', {}, { name });
            const result = {
                success: true,
    ...
    server.registerTool('meta_get_entities_by_label', {
        title: 'Buscar Entidades por Label',
        description: 'Lista campanhas, ad sets ou anúncios que possuem um label específico aplicado. ' +
        ...
        try {
            ...
            const entities = await paginate(`${label_id}/${entity_type}`, { fields: fields[entity_type] }, limit);
    ...
    server.registerTool('meta_get_account_activities', {
        title: 'Log de Atividades da Conta',
        description: 'Retorna o histórico de mudanças feitas na conta de anúncio: criação, edição e exclusão ' +
        ...
        try {
            ...
            const activities = await paginate(`${accountId}/activities`, params, limit);
    ...
    server.registerTool('meta_get_delivery_estimate', {
        title: 'Estimativa de Entrega do Ad Set',
        description: 'Estima o alcance diário, impressões e custo para um ad set com base no targeting e orçamento. ' +
        ...
        try {
            ...
            const response = await makeRequest(`${accountId}/delivery_estimate`, 'GET', params);
    ...
    server.registerTool('meta_get_minimum_budgets', {
        title: 'Orçamentos Mínimos por Objetivo',
        description: 'Retorna os valores mínimos de orçamento diário e total para cada objetivo de campanha ' +
        ...
        try {
            const accountId = resolveAccountId(ad_account_id);
            const response = await makeRequest(`${accountId}/minimum_budgets`, 'GET', { fields: 'currency,min_daily_budget_high_freq,min_daily_budget_imp,min_daily_budget_low_freq,min_daily_budget_video_views' });
            const data = response.data?.[0] ?? {};
            const currency = data.currency || 'BRL';
            const toCurrency = (v) => v ? `${(parseInt(v) / 100).toFixed(2)} ${currency}` : null;
            const result = {
                ad_account_id: accountId,
                currency,
