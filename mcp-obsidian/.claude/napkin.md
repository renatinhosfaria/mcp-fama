...
## Domain Notes
- **2026-05-07 — Auto-wikilinks por ID + upsert_hub**: 36 tools registrados (35 + `upsert_hub`). `upsert_lead_timeline`, `append_lead_interaction`, `upsert_broker_profile`, `append_broker_interaction` aceitam `client_id`, `broker_id`, `empreendimento_id|slug|name`, `fonte`, `regiao`. Resolver em `src/vault/entity-resolver.ts` mapeia ID→path canônico (filename-prefix lookup p/ client/broker, frontmatter `empreendimento_id` lookup p/ empreendimento, deterministic-by-slug p/ fonte/regiao). Hubs ausentes ganham stub `type: hub, status: stub` automático. Wikilinks renderizados como linha `Vínculos: [[a]] · [[b]]` no topo do body + array `wikilinks: [...]` no frontmatter. Tag normalization (`src/vault/tags.ts`) ASCII-fold + kebab-case + strip de `_YYYY-MM-DD` trailing — gera `tag_warnings` no response sem erro.
- **Lead/broker body parsers descartam tudo antes do primeiro `## section`** — injetar uma linha de prefixo no body é seguro durante append (que não re-serializa via parseLeadBody/serializeLeadBody) mas se perde em upsert (que re-parseia e re-serializa). Solução: regenerar a linha Vínculos a partir dos IDs em frontmatter em todo upsert.
- **HubSchema (frontmatter.ts) exige `scope` E `maintainer`** — fácil esquecer ao construir frontmatter de hub. Stubs gerados pelo resolver setam ambos automaticamente (`scope: <kind>`, `maintainer: <as_agent>`).
- **entity-profile e hub schemas usam `passthrough()`** — campos arbitrários (`client_id`, `wikilinks`, etc.) passam validação sem precisar adicionar ao Zod schema. Conveniente, mas typos em nomes de campo não falham loud.
